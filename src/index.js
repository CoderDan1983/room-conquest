...
function displayPlayer(parentElement, player, elementArray, action){ //action not used!!
    ...
    for(let b=0; b < player.branches.length; b++){
        ...
        //powerContainer.textContent = "powerContainer!"
    ...
    for(let a=0; a < array.length; a++){
        ...
        if(Array.isArray(miniAbilList)){
            for(let m=0; m < miniAbilList.length; m++){ //go through ability lists ^_^
                //if player doesn't have this ability
                if(findInArray(currentAbilities, miniAbilList[m].name) == false){
